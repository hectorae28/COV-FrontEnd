function transformBackendData(backendData) {
    const frontendData = {
      id: `sol-${Date.now()}`, // Generar un ID similar al frontend
  ...
  function transformItem(item, type) {
    return {
      ...
      nombre: type === "Constancia" 
        ? `Constancia: ${getConstanciaNombre(item.id)}`
        : type,
  ...
  function calcularTotal(detalles) {
    return [
      ...
      ...detalles.constancias.map(c => c.monto),
      detalles.especializacion?.monto || 0
    ].reduce((sum, monto) => sum + monto, 0);
  }
  
  function getDocumentosRequeridos(detalles) {
    // Implementar lógica para obtener documentos requeridos
    return ["Foto tipo carnet", "Comprobante de pago"];
  }
  
  function getDocumentosAdjuntos(detalles) {
    // Implementar lógica para obtener documentos adjuntos
    return Object.values(detalles.especializacion?.archivos || []);
  }
