...
export default function AppBar() {
  ...
  useEffect(() => {
    if (mobileMenuOpen) {
      menuItems.forEach((item, index) => {
        if (item.submenu && item.submenu.some(subItem => subItem.route && isSubmenuActive(subItem.route))) {
  ...
  return (
              ...
              {menuItems.map((item, index) => (
                ...
                >
                  {item.route ? (
                    ...
                    <span className={`text-white font-bold text-[12px] lg:text-[13px] xl:text-[15px] 2xl:text-[16px] cursor-pointer flex items-center gap-1 lg:gap-2 transition-all duration-300 whitespace-nowrap ${isMenuActive(item) ? 'text-yellow-400' : ''}`}>
                      {item.title}
                      {item.submenu && <FaChevronDown className={`w-2 h-2 lg:w-2.5 lg:h-2.5 xl:w-3 xl:h-3 transition-transform duration-500 ${openMenu === index ? 'rotate-180' : ''}`} />}
                    </span>
                  )}
                      ...
                      >
                        <div className="space-y-2 px-2">
                          {item.submenu.map((subItem, subIndex) => (
                            <div
                              key={subIndex}
                              onClick={() => navigateTo(subItem.route, subItem.action)}
          ...
          <div className="flex items-center space-x-2 md:space-x-4">
            ...
            <div className="hidden md:block lg:hidden">
              <div
                ...
                className={`bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-600 text-white font-bold text-[14px] py-2 px-4 rounded-full cursor-pointer transition-all duration-200 whitespace-nowrap ${pathname === "/colegiados" ? 'ring-2 ring-yellow-300' : ''}`}
              >
                Colegiados
              </div>
            </div>
            ...
            <div className="hidden lg:flex items-center">
              <div
                onClick={() => navigateTo("/colegiados")}
                className={`bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-600 text-white font-bold text-[12px] lg:text-[14px] xl:text-[16px] 2xl:text-[18px] py-1.5 lg:py-2 px-3 lg:px-4 xl:px-6 2xl:px-8 rounded-full cursor-pointer transition-all duration-200 whitespace-nowrap ${pathname === "/colegiados" ? 'ring-2 ring-yellow-300' : ''}`}
              >
                Colegiados
              </div>
            </div>
        ...
        >
          <div className="px-4 py-2">
                      ...
                      {item.submenu.map((subItem, subIndex) => {
                        ...
                        return (
                          <div
                            key={subIndex}
                            onClick={() => navigateTo(subItem.route, subItem.action)}
                            className={`py-2 cursor-pointer ${isSubActive ? 'text-yellow-300 font-bold' : 'text-white hover:text-gray-200'}`}
                          >
                            {subItem.title}
                          </div>
                        );
            ...
            <div className="py-4 flex justify-center md:hidden">
              <div
                onClick={() => navigateTo("/colegiados")}
                className={`bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-600 text-white font-bold text-[16px] py-2 px-6 rounded-full cursor-pointer transition-all duration-200 ${pathname === "/colegiados" ? 'ring-2 ring-yellow-300' : ''}`}
              >
                Colegiados
              </div>
            </div>
    ...
    {/* Logo Download Modal */}
    {showLogoModal && <LogoDownloadModal onClose={() => setShowLogoModal(false)} />}
  </>
);
}