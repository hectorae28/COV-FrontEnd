...
import SeleccionarTipoSolvencia from "./SeleccionarTipoSolvencia"

export default function CrearSolvenciaModal({
    onClose,
    onSolvenciaCreada,
    colegiados = [],
    ...
    // Estado para almacenar la solvencia creada
    const [solvenciaCreada, setSolvenciaCreada] = useState(null)

    // Función para manejar la creación de solvencia
    const handleSolvenciaCreada = (nuevaSolvencia) => {
        ...
        // Agregar la información del creador a la solvencia
        const solvenciaConCreador = {
            ...nuevaSolvencia,
            creador: {
                nombre: userInfo.name || userInfo.email?.split("@")[0] || "Usuario",
        ...
        setSolvenciaCreada(solvenciaConCreador)
        onSolvenciaCreada(solvenciaConCreador)
    }

    // Función para ir a la vista de detalle de la solvencia
    const handleVerDetalle = () => {
        if (onVerDetalle) {
            onVerDetalle(solvenciaCreada.id)
            onClose()
        } else {
            onClose()
        }
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                {solvenciaCreada ? (
                    <div className="p-6 md:p-8">
                        ...
                        <h3 className="text-xl font-bold text-center text-[#41023B] mb-4">¡Solvencia registrada correctamente!</h3>
                        <div className="bg-[#f8f9fa] p-5 rounded-xl border border-gray-200 mb-6">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <p className="text-sm text-gray-500">Tipo</p>
                                    <p className="font-medium text-gray-800">{solvenciaCreada.tipo}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500">Colegiado</p>
                                    <p className="font-medium text-gray-800">{solvenciaCreada.colegiadoNombre}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500">Referencia</p>
                                    <p className="font-medium text-gray-800">{solvenciaCreada.referencia}</p>
                                </div>
                                <div>
                                ...
                                {solvenciaCreada.creador && (
                                    <div className="sm:col-span-2">
                                        <div className="flex items-center justify-center md:justify-start">
                                            {solvenciaCreada.creador.esAdmin ? (
                                            ...
                                            <div>
                                                <p className="text-sm font-medium">{solvenciaCreada.creador.nombre}</p>
                                                <p className="text-xs text-gray-500">{solvenciaCreada.creador.email}</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
