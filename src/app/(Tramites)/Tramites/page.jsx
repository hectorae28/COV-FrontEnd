...
import { ArrowRight, Download, ExternalLink, CreditCard, ChevronDown } from "lucide-react"
import { BancoVenezuelaModal, PayPalModal } from "../../Components/Tramites/PaymentModals"
import { tramitesInfo, tramiteMapping, tarifasData } from "../../Components/Tramites/TramitesData"
...
export default function TramitesPanel() {
  ...
  const dropdownOptions = Object.entries(tramitesInfo).filter(([id]) => id !== activeTab)
  ...
  return (
    <div className="w-full px-4 md:px-20 py-6 md:py-12">
      ...
      <div
        className={`${!isMobile ? (activeTab === "avalCursos" ? "grid grid-cols-[1fr_4fr]" : "grid grid-cols-[1fr_3fr_1.5fr]") : "block"} gap-8`}
        ref={contentRef}
      >
            ...
            <div className="mb-6">
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background: `linear-gradient(to right, ${tramiteInfo.color}DD, ${tramiteInfo.color + "88"})`,
                  }}
                />
                ...
                <div className="absolute inset-0 px-4 flex items-center justify-between z-10">
                  <div className="flex items-center">
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-bold text-white drop-shadow-md">{tramiteInfo.title}</h3>
                    </div>
                  </div>
                  <div className={`text-white transition-transform duration-300 ${isDropdownOpen ? "rotate-180" : ""}`}>
                      ...
                      >
                        ...
                        <div className="absolute inset-0 px-4 flex items-center justify-between z-10">
                          <div className="flex items-center">
                            ...
                            <div className="flex-1">
                              <h3 className="text-base font-bold text-white drop-shadow-md">{info.title}</h3>
                            </div>
                          </div>
            ...
            <div className="flex flex-col gap-4">
              {Object.entries(tramitesInfo).map(([id, info], index) => (
                <motion.div
                  key={id}
                  className={`relative overflow-hidden rounded-xl shadow-lg cursor-pointer transition-all duration-300 group ${
                    activeTab === id ? "ring-2 transform scale-[1.02] z-10" : "hover:shadow-xl bg-white"
                  }`}
                  style={{
        ...
        <AnimatePresence mode="wait">
          <motion.div
            key={activeTab}
            className={`w-full bg-white rounded-xl shadow-xl overflow-hidden relative ${isMobile ? "mb-6" : ""}`}
                ...
                {tramiteInfo.pdfViewer ? (
                  <>
                    ...
                    <div className="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
                      <h4 className="font-medium mb-2" style={{ color: tramiteInfo.color }}>
                        Información para organizar cursos avalados por el COV
                      </h4>
                      <p className="text-gray-700 flex items-start">
                        <span className="text-gray-400 mr-2 font-medium mt-1">•</span>
                        <span>
                    ...
                    <div className="md:hidden flex flex-col gap-4 items-center">
                      <div className="w-full p-4 bg-gray-50 rounded-lg border border-gray-200 text-center">
                        <p className="text-gray-700 mb-4">
                          Para visualizar el documento de requisitos, puede abrirlo en una nueva ventana o descargarlo.
                        </p>
                        <div className="flex justify-center gap-4">
                          <button
                            onClick={() => window.open(tramiteInfo.pdfUrl, "_blank")}
                          ...
                          <a
                            ...
                            style={{ color: tramiteInfo.color, borderColor: tramiteInfo.color }}
                          >
                  ...
                  <>
                    ...
                    <div>
                      <h4 className="font-semibold text-lg mb-3" style={{ color: tramiteInfo.color }}>
                        Requisitos:
                      </h4>
                      <ul className="list-disc pl-5 md:pl-8">
                        {tramiteInfo.requisitos.map((req, index) => (
                          <li
                            key={index}
                            className={`text-gray-700 ${req.startsWith("-") ? "ml-4 list-none" : ""} mb-1`}
                          >
                            {req}
                          </li>
                        ))}
        ...
        {(activeTab !== "avalCursos" || !isMobile) && (
          <motion.div
            className={`w-full bg-white rounded-xl shadow-xl overflow-hidden ${isMobile && activeTab !== "avalCursos" ? "block" : ""}`}
            initial={{ opacity: 0, x: 30 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5 }}
            style={{ display: isMobile && activeTab === "avalCursos" ? "none" : "" }}
          >
            {/* Colored Top Border */}
            <motion.div
              className="h-1 w-full"
              ...
              >
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {tarifasData.map((item, index) => (
                          <tr
                            ...
                            style={{
                              backgroundColor:
                                tramiteMapping[activeTab] === item.tramite ? `${tramiteInfo.color}15` : "",
                              fontWeight: tramiteMapping[activeTab] === item.tramite ? "bold" : "normal",
                            }}
                          >
                            <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-700">{item.tramite}</td>
                            <td
                              className="px-4 py-2 whitespace-nowrap text-sm"
                              style={{
                                color:
                                  tramiteMapping[activeTab] === item.tramite ? tramiteInfo.color : "rgb(55, 65, 81)",
                              }}
                            >
                              {item.monto}
                            </td>
                ...
                <div className="text-sm text-gray-700 space-y-2 mb-4">
                  {activeTab === "odontologos" && (
                    <p>
                      Las inscripciones se cobran a partir de la fecha de la emisión del título. Consultar por vía
                      telefónica para obtener un monto exacto.
                    </p>
                  )}
                  <p className="text-justify">
                    <span className="font-bold">Carnet COV:</span> Para obtener el carnet (físico o digital), es
                    necesario estar solvente con el COV. El físico se retira en las oficinas y el digital se descarga
                    desde el Sistema Colegiados.
                  </p>
